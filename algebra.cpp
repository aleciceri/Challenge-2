...
void Matrix<T,S>::resize(std::size_t new_nrows, std::size_t new_ncols){
    ...
    if(S==StorageOrder::Row){
        for(auto i=map.begin();i<map.end();i++){
            std::size_t new_row=<std::size_t>(i.first[0]*i.first[1]/new_ncols);
            std::size_t now_col=<std::size_t>(i.first[0]*i.first[1]%new_ncols);
        ...
        for(auto i=map.begin();i<map.end();i++){
            std::size_t new_col=<std::size_t>(i.first[0]*i.first[1]/new_nrows);
            std::size_t now_row=<std::size_t>(i.first[0]*i.first[1]%new_nrows);
            std::array<std::size_t,2> pos={new_row,new_col};
            elements_new[pos]=i.second;
        }        
    }
    elements.clear();
    elements=element_new;
    nrows=new_nrows;
    ncols=new_ncols;
    return;
}
    ...
    if(S==StorageOrder::Row){
        if(M.compressed){
            for (std::size_t i = 0; i < nrows; i++)
            {
                ...
                {
                    element+=value[j]*vec[M.second_indexes[j]];
                }
                result[i]=element;
            }
        }
